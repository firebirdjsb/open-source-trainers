...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        if (bd->pTexUploadBuffer == nullptr || upload_size > bd->pTexUploadBufferSize)
        {
            ...
            [[maybe_unused]] HRESULT hr = bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc,
        ...
        [[maybe_unused]] HRESULT hr = cmdList->Close();
        IM_ASSERT(SUCCEEDED(hr));
...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    [[maybe_unused]] HRESULT hr = ::CreateDXGIFactory1(IID_PPV_ARGS(&bd->pdxgiFactory));
    IM_ASSERT(hr == S_OK);
